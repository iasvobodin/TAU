...
# Параметры (задаются прямо здесь)
$ApiUrl = "http://10.69.19.59:3000"
$ApiKey = "your-secret-api-key-12345"
$ReportsPath = "\\rucekaspinffs05.metran.local\Dept-MP\Production\Internal\Продукты\ТАУ\Результаты"
...
try {
    Write-Info "Запрос: GET $ApiUrl/products"
...
foreach ($product in $moduleProducts) {
    ...
    try {
        $detailResponse = Invoke-WebRequest -Uri "$ApiUrl/products/$($product.snProduct)" `
            -Method GET `
            -Headers $headers `
            -UseBasicParsing `
            -ErrorAction Stop
...
try {
    if (-not (Test-Path -Path $ReportsPath -PathType Container)) {
        throw "Сетевой путь недоступен: $ReportsPath"
    }

    Write-Info "Путь: $ReportsPath"
