...
import { VTable, VTextarea, VRadioGroup, VRadio, VBtn, VCard, VCardTitle, VCardText } from 'vuetify/components';
...
const isSaveDisabled = computed(() => {
  ...
  const allFieldsFilled = template.value.fields.every(
    (field) => values.value[field.name]?.status === 'pass' || values.value[field.name]?.status === 'fail'
  );
  // Проверяем, что для всех полей со статусом fail есть комментарий
  const allFailFieldsCommented = template.value.fields.every(
    (field) => values.value[field.name]?.status !== 'fail' || (values.value[field.name]?.comment || '').trim() !== ''
  );
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="(field, index) in template.fields" :key="index">
            <td width="60%">{{ field.name }} <h3 class="text-red" v-if="  values[field.name].status === 'fail' &&
                    values[field.name].comment === ''">Комментарий обязателен!</h3 class="text-red"></td>
            <td width="20%">
