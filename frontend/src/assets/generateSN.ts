...
// }
import type { ModulesType } from './interfaces'

const SERIAL_NUMBER_CONFIG = {
  prefix: 'TAU',
  uniqueNumberLength: 6,
  moduleType: {
    Controller: 1,
    PowerSupply: 2,
    Modules: 3,
...
export function genSN(type: ModulesType, quantity: number, startUniqueNumber: number): string[] {
  if (quantity <= 0) {
    throw new Error('Quantity must be greater than 0')
  }
  if (startUniqueNumber < 0) {
    throw new Error('Start unique number must be non-negative')
  }

  const { week, year } = getCurrentWeekAndYear()
  const typeCode = SERIAL_NUMBER_CONFIG.moduleType[type]
  const serialNumbers: string[] = []

  for (let i = 0; i < quantity; i++) {
    const uniqueNumber = (startUniqueNumber + i + 1)
      .toString()
      .padStart(SERIAL_NUMBER_CONFIG.uniqueNumberLength, '0')
    const serialNumber = `${SERIAL_NUMBER_CONFIG.prefix}${week}${year}${typeCode}${uniqueNumber}`
